...
const schema = joi.object({
    ...
    Status: joi.string().required().min(1).max(45),
...
exports.patchTask = function patchTask(req, res) {
    ...
    const query = "UPDATE Tasks SET Text = ?, Status = ?, ListID = ? WHERE ID = ? " 
    const values = [req.body.Text, req.body.Status, req.body.ListID,req.body.ID]
